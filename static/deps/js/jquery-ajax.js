...
$(document).ready(function () {
    ...
    function updateQuantity(cartId, newQuantity) {
        $.ajax({
            url: '/cart/change/',  // Пример URL, проверьте его в вашем проекте
            type: 'POST',
            data: {
                cart_id: cartId,
                quantity_change: newQuantity,  // передаем изменение количества, а не только новое
                csrfmiddlewaretoken: $('meta[name="csrf-token"]').attr('content')  // защита от CSRF
            },
            success: function(response) {
                if (response.cart_items_html) {
                    // Обновляем поле с количеством товара
                    $('input[data-cart-id="'+cartId+'"]').val(newQuantity);  
    ...
    function removeFromCart(cartId) {
        $.ajax({
            url: '/cart/remove/',  // Пример URL для удаления товара
            type: 'POST',
            data: {
                cart_id: cartId,
                csrfmiddlewaretoken: $('meta[name="csrf-token"]').attr('content')  // защита от CSRF
    ...
    //     });
    // }
   // Здесь можно хранить логику работы с AJAX запросами
